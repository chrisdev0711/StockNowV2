    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <x-nav-dropdown title="Suppliers" align="right" width="48">
                    ...
                    @endcan
                    @can('view-any', App\Models\Supplier::class)
                    <x-dropdown-link href="{{ route('suppliers.importView') }}">
                    Import Suppliers
                    </x-dropdown-link>
                    @endcan
                </x-nav-dropdown>
                <x-nav-dropdown title="Products" align="right" width="48">
                        @can('view-any', App\Models\Product::class)
                        <x-dropdown-link href="{{ route('products.index') }}">
                        Stock List
                        </x-dropdown-link>
                        @endcan
                        @can('view-any', App\Models\Sellable::class)
                        <x-dropdown-link href="{{ route('sellables.index') }}">
                        Sellables (Till Products)
                        </x-dropdown-link>
                        @endcan
                        @can('view-any', App\Models\Product::class)
                        <x-dropdown-link href="{{ route('products.importView') }}">
                        Import Ingredients
                        </x-dropdown-link>
                        @endcan
                </x-nav-dropdown>
                <x-nav-dropdown title="Stocktakes" align="right" width="48">
                    @can('view-any', App\Models\StockTake::class)
                    <x-dropdown-link href="{{ route('stockTakes.filter') }}">
                    Create New Stocktake
                    </x-dropdown-link>
                    @endcan
                    @can('view-any', App\Models\StockTake::class)
                    <x-dropdown-link href="{{ route('stockTakes.history') }}">
                    Stocktake History
                    </x-dropdown-link>
                    @endcan
                </x-nav-dropdown>
                <x-nav-dropdown title="Orders" align="right" width="48">
                    ...
                    @endcan
                    @can('view-any', App\Models\Order::class)
                    <x-dropdown-link href="{{ route('orders.receive') }}">
                    Receive Order
                    </x-dropdown-link>
                    @endcan
                    @can('view-any', App\Models\Order::class)
                ...
                <x-nav-dropdown title="Reports" align="right" width="48">
                    @can('view-any', App\Models\Product::class)
                    <x-dropdown-link href="{{ route('products.index') }}">
                    Stock List
                    </x-dropdown-link>
                    @endcan
                    @can('view-any', App\Models\Product::class)
                    <x-dropdown-link href="{{ route('products.newindex') }}">
                    Site Stock Report
                    </x-dropdown-link>
                    @endcan
                    @can('view-any', App\Models\Sellable::class)
                ...
                <x-nav-dropdown title="Square" align="right" width="48">
                    <x-dropdown-link href="{{ route('square.fetchVatRates') }}">
                        Import Vat Rates
                    </x-dropdown-link>
                    <x-dropdown-link href="{{ route('square.fetchCategories') }}">
                        Import Categories
                    </x-dropdown-link>
                    <x-dropdown-link href="{{ route('square.fetchLocations') }}">
                        Import Locations
                    </x-dropdown-link>
                    <x-dropdown-link href="{{ route('square.fetchSellables') }}">
                        Import Sellables
                    </x-dropdown-link>
                    <x-dropdown-link href="{{ route('square.fetchSales') }}">
                        Import Transactions
                    </x-dropdown-link>
                </x-nav-dropdown>
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                ...
                <!-- Site Dropdown -->
                <div class="ml-3 relative">
                    @php $activeSite = Auth::user()->activeLocation() @endphp
                    @if($activeSite)
                    <livewire:active-site-menu
                        :active="$activeSite"
                    />
                    @endif
                </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-jet-responsive-nav-link href="{{ route('dashboard') }}" :active="request()->routeIs('dashboard')">
                ...
                @endcan
                @can('view-any', App\Models\Zone::class)
                <x-jet-responsive-nav-link href="{{ route('zones.index') }}">
                Zones
                </x-jet-responsive-nav-link>
                @endcan
                @can('view-any', App\Models\ProductCategory::class)
                ...
                @endcan
                @can('view-any', App\Models\Sellable::class)
                <x-jet-responsive-nav-link href="{{ route('sellables.index') }}">
                Sellables
                </x-jet-responsive-nav-link>
                @endcan
